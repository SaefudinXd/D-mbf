		...
		try:
			...
			for softek in memek:
				if "&amp;refid=" in softek[0]:
					self.id.append(re.findall("id=(.*?)&",softek[0])[0]+"[Ipul]"+softek[1])
				elif "profile.php?" in softek[0]:
					self.id.append(re.findall("id=(.*)",softek[0])[0]+"[Ipul]"+softek[1])
				elif "?refid=" in softek[0]:
					self.id.append(re.findall("(.*?)\?refid=",softek[0])[0]+"[Ipul]"+softek[1])
				else:
					self.id.append(softek[0]+"[Ipul]"+softek[1])
				...
				if "profile.php?" in softek[0]:
					...
					if "/" in tol[-1:]:
						...
						if tol in self.id:
							...
							self.id.append(tol+"[Ipul]"+softek[1])
				else:
	...
	def login(self):
		try:
			...
			if "mbasic_logout_button" in cek:
				...
				from kentod_cwek import Ipul,informasi
				if "Laporkan Masalah" in cek:
					mengontol=Ipul.ganteng(zxss(self.cok),self.url)
					informasi.info(zxss(self.cok),cek).myinfo()
					mengontol.reaksi()
					#os.system('xdg-open https://wa.me/6289515127742?text=Hai-bang')
					#waktu(1.09)
...
def user():
    ...
    os.system('am start https://wa.me/6289515127742?text=bang+saya+mau+beli+premium+nya+ini+licensei+premium+saya+licensei:%20' + id + ' >/dev/null')
    time.sleep(1)
    os.sys.exit()
def user1():
    try:
      ...
      r = requests.get("https://github.com/SaefudinXd/SaefudinXd/blob/main/id.txt").text
      if j in r:
          ...
          input ('  Press Enter To Chat Admin');time.sleep(0.07)
          os.system('am start https://wa.me/6289515127742?text=bang+saya+mau+beli+premium+nya+ini+licensei+premium+saya+licensei:%20' + j + ' >/dev/null')
          os.sys.exit()
...
def wa():
	os.system('xdg-open https://wa.me/6289515127742?text=bagi+password+nya+bang')
